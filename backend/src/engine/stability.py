...
class StabilityAnalyzer:
    def analyze(self, events: List[dict]) -> dict:
        ...
        classification, reason = self._classify(means, yearly["count"].values)
    ...
    def _classify(self, means: np.ndarray, counts: np.ndarray) -> tuple:
        if len(means) < 2:
            return "Regime-Dependent", "樣本年份不足，無法評估穩定性"

        overall_mean = np.mean(means)

        # Coefficient of variation (CV) on yearly means
        if abs(overall_mean) < 0.5:
            # Near-zero mean: use absolute std instead
            cv = np.std(means)
        else:
            cv = abs(np.std(means) / overall_mean)

        # Check sign consistency
        positive_years = np.sum(means > 0)
        negative_years = np.sum(means < 0)
        total_years = len(means)
        sign_ratio = max(positive_years, negative_years) / total_years

        if cv < 0.6 and sign_ratio >= 0.75:
            return "Stable", "各年份報酬分布一致，歷史表現穩定"
        elif cv < 1.5 or sign_ratio >= 0.6:
            return "Regime-Dependent", "報酬特徵與市場環境高度相關，需評估當前環境適配度"
        else:
            return "Unstable", "歷史表現高度波動，過去統計特徵在未來可能不可靠"
